...
describe("ensureServerRunning (detached)", () => {
  test(
    "spawns a detached subprocess — PID is NOT the test process PID",
    async () => {
  ...
  test(
    "PID file points to the subprocess, not the parent",
    async () => {
      const info = await ensureServerRunning(makeCfg());
      spawnedPids.push(info.pid);
  ...
  test(
    "spawned server responds to HTTP",
    async () => {
      const info = await ensureServerRunning(makeCfg());
      spawnedPids.push(info.pid);
  ...
  test(
    "idempotent — second call returns same server, no new spawn",
    async () => {
  ...
  test(
    "re-spawns when previous PID is dead (stale PID file)",
    async () => {
  ...
  test(
    "killing the spawned subprocess does not kill the test process",
    async () => {
      const info = await ensureServerRunning(makeCfg());
      spawnedPids.push(info.pid);
